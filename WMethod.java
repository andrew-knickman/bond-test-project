...
public class WMethod{
   ...
   */
   public static void main(String [] args){
     ...
     //Run test cases against FSM
     char[] spltTestCase; //character array of test case
     String strgTestCase; //string to concat test case chars
     for(int x = 0; x < tests.size(); x++)
     {
    	 spltTestCase = tests.elementAt(x).toCharArray();
    	 strgTestCase = "";
    	 System.out.println("\nTest Case " + (x+1));
    	 for(int y = 0; y < spltTestCase.length; y++)
    	 {
    		 strgTestCase = strgTestCase + " " + spltTestCase[y];
    		 System.out.print(spltTestCase[y] + " ");
    	 }
    	 Utilities.runFSM(FSM, 1, strgTestCase, " ", false);
     }
     
     //Generate JUnit test methods for all test cases generated by W Method tool
    for(int m = 0; m < tests.size(); m++)
    {
		 spltTestCase = tests.elementAt(m).toCharArray();
		 strgTestCase = "";
    	...
    	System.out.println("@Test\npublic void testCase" + (m+1) + "() throws Exception{");
    	String outputStr = Utilities.runFSM(FSM, 1, strgTestCase, " ", true);
    	System.out.println("\tJamesBond fixture = new JamesBond()");
    	System.out.println("\tboolean result = fixture.bondRegex(\"" + tests.elementAt(m) + "\");");
    	if(outputStr.contains("yes"))
    		System.out.println("\tassertTrue(fixture.bondRegex(result));");
    	else
    		System.out.println("\tassertFalse(fixture.bondRegex(result);");
    	System.out.println("}");
    }
